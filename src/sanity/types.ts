...
export type TabIcon = {
	asset?: SanityImageAssetReference
	media?: unknown // Unable to locate the referenced type "tab.icon.media" in schema
	hotspot?: SanityImageHotspot
	crop?: SanityImageCrop
	_type: 'image'
}
...
// Variable: PAGE_MD_QUERY
// Query: *[_type == 'page' && metadata.slug.current == $slug][0].markdown.code
export type PAGE_MD_QUERY_RESULT = string | null

// Source: src/app/(frontend)/api/md/[...slug]/route.ts
// Variable: BLOG_POST_MD_QUERY
// Query: *[_type == 'blog.post' && metadata.slug.current == $slug][0].markdown.code
export type BLOG_POST_MD_QUERY_RESULT = string | null
...
// Query: *[_type == 'blog.post' && !(_id in $featuredIds)]|order(publishDate desc){		...,			'readTime': length(string::split(pt::text(content), ' ')) / 200,	categories[]->{		title,		slug	},	author->{		name,		image{			...,			asset->		}	},		'slug': $blogDir + metadata.slug.current,	}
...
// Source: src/modules/blog-index/index.tsx
// Variable: BLOG_FEATURED_QUERY
// Query: *[_type == 'blog.post' && _id in $featuredIds]{		...,			'readTime': length(string::split(pt::text(content), ' ')) / 200,	categories[]->{		title,		slug	},	author->{		name,		image{			...,			asset->		}	},		'slug': $blogDir + metadata.slug.current,	}
export type BLOG_FEATURED_QUERY_RESULT = Array<{
	_id: string
	_type: 'blog.post'
	_createdAt: string
	_updatedAt: string
...
// Source: src/modules/blog-post-list/index.tsx
// Variable: BLOG_POST_LIST_QUERY
// Query: *[_type == 'blog.post']|order(publishDate desc)[0...$limit]{		...,			'readTime': length(string::split(pt::text(content), ' ')) / 200,	categories[]->{		title,		slug	},	author->{		name,		image{			...,			asset->		}	},		'slug': $blogDir + metadata.slug.current,	}
...
// Source: src/modules/search-module/actions.ts
...
// Source: src/ui/blog/filter-list.tsx
