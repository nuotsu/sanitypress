...
// Query: *[_type == 'page'		&& defined(metadata.slug.current)		&& metadata.noIndex != true		&& metadata.slug.current != '404'		&& length(markdown.code) > 0	] | order(metadata.slug.current != 'index', metadata.slug.current asc) {		'title': coalesce(metadata.title, metadata.slug.current),		'slug': metadata.slug.current,		'description': metadata.description,	}
...
// Query: *[_type == 'blog.post'		&& defined(metadata.slug.current)		&& metadata.noIndex != true		&& length(markdown.code) > 0	] | order(publishDate desc) {		'title': coalesce(title, metadata.title),		'slug': $blogDir + '/' + metadata.slug.current,		'description': metadata.description,	}
